    ...
    var map = (function () { // draw map using similar method to level drawing
      ...
      return function (mapData) {
        ...
        if (mouse.y > 0 && mouse.y < vertical * 16) { // if within y bounds of map, allow scroll
          if (mouse.x > scrollRight && mouse.x < gameSettings.width) offset += ms / 5 + (mouse.x - scrollRight) / 25; // <-- frame interval used to get same move speed on any monitor refresh rate (rpg-test esports gaming team with 300hz monitors soon??)
          if (mouse.x < scrollLeft && mouse.x > 0 && offset > 0) offset -= ms / 5 - (mouse.x - scrollLeft) / 25;
        }
