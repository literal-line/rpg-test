...
var RPG_TEST = (function () {
	...
	var gameLoop = (function () {
		...
		var title = (function () {
			...
			return function () {
				drawText({ text: gameSettings.version, color: 20, size: 16, x: 0, y: gameSettings.height });
				drawText({ text: 'a game by Literal Line', color: 14, size: 16, x: gameSettings.width / 2, y: 115, center: true });
		...
		var credits = (function () {
			...
			return function () {
				drawText({ text: 'Credits', color: 14, size: 54, x: gameSettings.width / 2, y: 80, center: true });
				drawText({ text: '• Coding and game engine by Literal Line', x: 15, y: gameSettings.height / 2 });
				drawText({ text: '• Sound effects from Dan Ball\'s', x: 15, y: gameSettings.height / 2 + 30 });
				drawText({ text: '• Color pallete from lospec.com', x: 15, y: gameSettings.height / 2 + 60 });
				drawText({ text: '  (slightly modified)', x: 15, y: gameSettings.height / 2 + 90 });
		...
		var newGame = (function () {
			...
			return function () {
				...
				for (var b in buttons.members) {
					...
					drawText({ text: teamData['member' + memberNumber].class, size: 16, x: cur.x, y: cur.y + 50, center: true });
					cur.draw();
				}
