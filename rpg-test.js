  ...
  var gameLoop = (function () {
    ...
    var map = (function () { // draw map using similar method to level drawing
      ...
      return function (mapData) {
        ...
        if (mouse.x > gameSettings.width - 175 && mouse.x < gameSettings.width && mouse.y > 0 && mouse.y < vertical * 16 + 16) offset += ms / 2; // <-- frame interval used to get same move speed on any monitor refresh rate (rpg-test esports gaming team with 300hz monitors soon??)
        if (mouse.x < 175 && mouse.x > 0 && offset > 0 && mouse.y > 0 && mouse.y < vertical * 16 + 16) offset -= ms / 2; // same frame interval method as above ^^^
